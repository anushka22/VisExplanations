...
Given a user-selected data relationship that captures the set of dependent variables of interest, we seek to progressively introduce variables that help explain the patterns seen in the visual representations. For simplicity, we describe a scenario where the user selects a bivariate relationship of interest and visualizes it as a scatterplot. The proposed method could be applied to other view types and associated quality metrics. Adding explanatory variables partitions the scatterplot into multiple plots (small multiples) such that there as many plots as there are discrete categories of a categorical Partitioner or discrete bins of a quantitative Partitioner variable. Therefore, a Partitioner splits or facets a particular visual representation into similar views of subsets of the data. We propose a number of criteria to evaluate the goodness of the splits resulting from adding a Partitioner variable to explain the structure seen in the original visual representation.

\subsection{Split Criteria}
We can handle the complexity of multi-dimensional data analysis by applying the facet operator to conditionally subset data based on a particular dimension across multiple views. Dividing data between views encodes the association between items using spatial proximity which impacts the patterns that are visible.
...
Choose a T.S. and establish a rejection rule
that will distinguish the null from the alternative.
3. Compute the T.S. for the original observations.
4. Rearrange the observations.
1. Compute T.S. for the rearranged data.
2. Compare the T.S. from original observation with the
ones from re-arranged data
