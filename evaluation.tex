...
Our first criterion is to prefer small multiple displays that have visually salient patterns. Our algorithm achieves this by using the input cognostic to measure the presence and extent of a visual pattern in the component plots of a candidate small multiple display. This approach works for arbitrary cognostic measures on any type of visualization, allowing us to create small multiple displays targeted at the interests of the analyst. 
 ...
 For the Boston dataset, the example in Section~\ref{sec:method} shows the highest ranked variable (“distance to an employment center”) on the Monotonic scagnostic. The highest ranked partitioning variable changes with the choice of scagnostic – the Outlying scagnostic picks “\% lower status of the population” seen in Figure~\ref{fig:bostonOutlying} and the Clumpy scagnostic picks the boolean variable “close to Chas river” seen in Figure~\ref{fig:bostonClumpy}. These partitioned views reveal different visually salient features about the original view.
